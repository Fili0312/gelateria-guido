...
await ctx.addCookies([{ name: 'gelateria_session', value: readFileSync(process.argv[2],'utf8').trim(), domain: 'localhost', path: '/' }]);
...
await p.evaluate(() => fetch('/gelateria/api/orders/current', { method: 'DELETE', headers: { Accept: 'application/json' } }));
