...
async function main() {
  ...
  esito(
    dopo.perFornitore
      .flatMap((g) => g.righe)
      .every((r) =>
        new Decimal(r.lineTotalNet).equals(new Decimal(r.priceNet).mul(r.quantityPacks).toDecimalPlaces(2)),
      ),
  ...
  console.log(`     ${riordino.copiate} copiate · ${riordino.cambiate.length} cambiate · ${riordino.saltate.length} saltate`);
  ...
  esito(
    riordino.cambiate.length === 2,
    `segnala i prezzi cambiati (${riordino.cambiate.length})`,
  );
  esito(
    riordino.cambiate.every((x) => new Decimal(x.prezzoAdesso).equals(new Decimal(x.prezzoAllora).mul(2))),
  ...
  esito(
    bozza.righe.every((r) => {
      ...
      return new Decimal(r.priceNet).equals(new Decimal(nuovo.currentPrice!.priceNet.toString()).mul(2));
    }),
  ...
  const tutti = await ordini.elenco({ q: '', stato: 'tutti', supplierId: '', giorni: 0, pagina: 1, perPagina: 20 });
  esito(tutti.totale === 7, `${tutti.totale} ordini in tutto`);

  const paginati = await ordini.elenco({ q: '', stato: 'tutti', supplierId: '', giorni: 0, pagina: 2, perPagina: 3 });
  esito(paginati.items.length === 3, `la seconda pagina da tre ne ha tre (${paginati.items.length})`);
  ...
  const dentro = await ordini.elenco({ q: '', stato: 'tutti', supplierId: a.supplierId, giorni: 0, pagina: 1, perPagina: 20 });
  esito(dentro.totale === 7, `filtrando per il fornitore degli ordini escono i suoi 7 (${dentro.totale})`);
  ...
  if (estraneo) {
    const fuori = await ordini.elenco({ q: '', stato: 'tutti', supplierId: estraneo.id, giorni: 0, pagina: 1, perPagina: 20 });
    esito(fuori.totale === 0, `e per un fornitore che non c'è (${estraneo.name}) non esce niente (${fuori.totale})`);
  }

  const nomeCercato = prima.perFornitore[0]!.righe[0]!.name.split(' ')[0]!;
  const perProdotto = await ordini.elenco({ q: nomeCercato, stato: 'tutti', supplierId: '', giorni: 0, pagina: 1, perPagina: 20 });
  esito(perProdotto.totale > 0, `la ricerca per prodotto («${nomeCercato}») trova ${perProdotto.totale} ordini`);
  const inesistente = await ordini.elenco({ q: 'zzz-non-esiste', stato: 'tutti', supplierId: '', giorni: 0, pagina: 1, perPagina: 20 });
  esito(inesistente.totale === 0, 'e un prodotto che non c\'è non ne trova nessuno');

  const recenti = await ordini.elenco({ q: '', stato: 'tutti', supplierId: '', giorni: 30, pagina: 1, perPagina: 20 });
  esito(recenti.totale === 7, 'il filtro per periodo li prende tutti (sono di oggi)');

  console.log('\n── Annullare, senza cancellare ─────────────────────────────────\n');

  const annullato = await ordini.annulla(confermato.orderId);
  ...
  const dueVolte = await ordini.annulla(confermato.orderId);
  esito(dueVolte.cancelledAt === annullato.cancelledAt, 'annullarlo due volte non cambia la data');
  const soloAnnullati = await ordini.elenco({ q: '', stato: 'CANCELLED', supplierId: '', giorni: 0, pagina: 1, perPagina: 20 });
